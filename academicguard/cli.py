...
def show_services():
    ...
    table = Table(title="Local Detection Components", show_header=True, header_style="bold blue")
    table.add_column("Component")
    table.add_column("Status")
    table.add_column("Note", style="dim")

    for s in services:
        status = "[green]AVAILABLE[/green]" if s.available else "[red]NOT INSTALLED[/red]"
        table.add_row(s.name, status, s.note)
    console.print(table)
    console.print()
    console.print("[dim]All detection runs locally -- no external APIs required.[/dim]")
    console.print("[dim]Run: academicguard setup-env  for installation instructions.[/dim]")
